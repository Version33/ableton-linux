...
# skipped the anchor hid the version-4 migration from every later run.
# Version 6 writes the unit's ExecStart through the stable runtime link rather
# than copying the shipped path: on a Nix install that path is a store path,
# which every upgrade renames and a garbage collection can delete. sudo is used
# only to open the firewall port when a firewall is active and to remove the old
# hook where one exists.
set -euo pipefail
