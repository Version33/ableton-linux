#!/usr/bin/env bash
# Build linkprobe.exe (Wine-side Ableton Link multicast verdict) the same way
# as build_midihot.sh: real PE via clang, Wine headers + import libs, no CRT.
set -e
cd "$(dirname "$0")"
...
S=$BLD/dlls/ws2_32/x86_64-windows
...
clang -target x86_64-windows-gnu -fuse-ld=lld --no-default-config \
  ...
  -o linkprobe.exe linkprobe.c \
  -L "$S" -L "$U" -L "$K" -L "$N" \
  -lws2_32 -luser32 -lkernel32 -lntdll
echo "built linkprobe.exe"
